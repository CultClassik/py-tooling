...
class FileSort():
  ...
  # replace self.chars_to_strip with a space
  def strip_file_name(self, file_name):
    file_name = file_name.casefold()
    for item in self.chars_to_strip:
      file_name = file_name.replace(item, ' ')
    return file_name

  def cleanup_dirs(self, path):
    # Create a List    
  ...
  def get_folders_list(self, path):
    folders = {}
  ...
  def get_matches(self, folder_name, files_list):
    matched_files = []
    ...
    for full_file_name in files_list:
      ...
      result = self.inspect_file(folder_strings, file_name)
      ...
      #   if word not in self.chars_to_strip:
  ...
  def inspect_file(self, folder_strings, file_name):
    trimmed_file_name = self.strip_file_name(file_name)
    ...
    for item in folder_strings:
      ...
      if folder_name_part not in self.chars_to_strip and folder_name_part in file_name_strings:
        word_matches += 1
  ...
  def move_files(self, target_path, files_list):
    for file_path in files_list:
      ...
      if os.path.isfile(file_dest):
        ...
        if src_file_size == dst_file_size:
          ...
          file_dest = target_path + "/" + file_name_split[0] + "-" + self.random_string_generator() + file_name_split[1]
...
if __name__ == '__main__':
    ...
    fsorter = FileSort(files_path, folders_path)
    items = fsorter.get_folders_list(folders_path)
    files_list =fsorter. get_files_list(files_path)
    ...
    for folder in list(items):
      files = fsorter.get_matches(folder, files_list)
      if len(files) > 0:
        items[folder] = files
      else:
    ...
    if num_args == 4 and sys.argv[3] == 'move':
      ...
      for folder, files in items.items():
        fsorter.move_files(folders_path + "/" + folder, items[folder])
      print('Cleaning up empty folders..')
      fsorter.cleanup_dirs(files_path)
    else:
